...
```js
const canvas_upper = (() => {
const canvas = contextWebGL(width, height);
...
```js
const zipreader = (fileDescriptor, { type = "string" } = {}) =>
  fileDescriptor
    .arrayBuffer()
...
```js
const archive_reader = await fetch(archive_select).then((z) => zipreader(z, { type: "arraybuffer" }))
```

```js
const tiff_file_buffer = await archive_reader.get("data/LandCover_NP_" + extractYearFromURL(archive_select) + ".tif")
```

```js
const tiff = await GeoTIFF.fromArrayBuffer(tiff_file_buffer)
```
...
```js
const image_data = await image.readRasters({ width, height })
```

```js
const imageAspectRatio = image.getWidth() / image.getHeight()
```
...
```js
const plotty = await import("https://cdn.skypack.dev/plotty@0.4.9?min")
```

```js
const featureArray = image_data[0].map((d) => (feature.includes(d) ? 1 : 0))
```
